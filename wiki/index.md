# Marketing Second Brain Demo Index

This index routes agents and readers through the marketing-only demo corpus.

## Mental Model

```text
intentional captures -> raw/intentional -> compiled marketing wiki
last30days sweeps    -> raw/sweeps      -> staging digest -> optional wiki promotion
queries              -> QMD or rg       -> wiki + raw citations
```

## Domains

| Domain | Description | Updated |
|---|---|---|
| Marketing | AI-native marketing workflows: source capture, SEO/AEO/GEO, paid creative ops, autonomous pages, content ops, lifecycle, analytics, and enablement | 2026-06-19 |

## Marketing

| Article | Summary | Updated |
|---|---|---|
| [Agentic Marketing Workflows](marketing/agentic-marketing-workflows.md) | Hub for marketing-centric AI as workflow transformation: reusable loops with sources, brand/product context, tool access, validation, distribution, and feedback. | 2026-06-19 |
| [SEO/AEO/GEO Content Systems](marketing/seo-aeo-geo-content-systems.md) | Machine-executable search and answer-visibility content systems: keyword/SERP research, briefs, MDX production, validation, and measurement. | 2026-06-19 |
| [Performance Marketing Creative Ops](marketing/performance-marketing-creative-ops.md) | Paid-media creative operations: ad exports, weak-asset diagnosis, audience/list building, platform constraints, approval gates, and feedback loops. | 2026-06-19 |
| [Autonomous Websites And Landing Pages](marketing/autonomous-websites-and-landing-pages.md) | Campaign-specific generated pages from account, keyword, paid, social, or rep signals, with review, distribution, and conversion measurement. | 2026-06-19 |
| [Content Ops And Editorial Systems](marketing/content-ops-and-editorial-systems.md) | Editorial and product-marketing systems for brand voice, source-backed drafting, case studies, scripts, launch briefs, and repurposing. | 2026-06-19 |
| [Lifecycle CRM And Marketing Ops](marketing/lifecycle-crm-and-marketing-ops.md) | Lifecycle, CRM, lead scoring, nurture, routing, consent, audit trails, and marketing-to-sales handoff workflows. | 2026-06-19 |
| [UGC And Creator Systems](marketing/ugc-and-creator-systems.md) | AI UGC and creator-video systems for product ads, TikTok/Reels formats, synthetic creator workflows, and governance risks. | 2026-06-19 |
| [Marketing Analytics And Enablement](marketing/marketing-analytics-and-enablement.md) | Campaign analytics plus marketing enablement metrics: workflow transformation, cohort maturity, adoption, and self-sufficiency. | 2026-06-19 |

## Retrieval Notes

- Use `wiki/` first for "what do I know about..." questions.
- Use `raw/` first for exact source recall, URLs, handles, or remembered phrases.
- Use `staging/` for recent-signal leads and incomplete captures, but mark uncertainty.

